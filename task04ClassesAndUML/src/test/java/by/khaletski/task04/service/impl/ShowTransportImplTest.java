...
class ShowTransportImplTest {

    @Test
    void showTransportTest() {
        ...
        sorted.addTour(new Tour(Type.HEALING, Location.TURKEY, Meals.THREE, Transport.AIRPLANE,
                10, 700));
        assertEquals(ServiceFactory.getInstance().getShowTransportServiceImpl()
                .showTransport(unsorted, Transport.AIRPLANE), sorted);
    }
}
